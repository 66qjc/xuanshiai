...
_SYSTEM_HEADER = (
    ...
    "4. 要具体：把字段落到场景里。例如爬山+摄影，不是「热爱户外与艺术」，"
...
_JSON_FORMAT_INSTRUCTION = (
    ...
    "理想型画像直接描述那个人的气质，如\"温柔稳定且拥有自己世界的人\"，"
    "不要加「理想伴侣：」这类前缀。\n"
    "- persona_tags：3-5个标签词，像朋友聊天时会用的说法，避免书面腔。\n"
    "- insight：50-150字，写「你」在意的本质，而不是条件本身。对照示例——\n"
    "  机械写法（禁止）：「该用户年龄26-33岁、本科以上学历、身高158cm以上，"
    "追求稳定的婚姻关系。」\n"
    "  人性化写法（目标）：「比起一张条件清单，你更在意那个人有没有把生活过明白——"
    ...
    "不要输出 30/25/20/15/10 这种等差梯度；各项独立取值，总和不必凑成100。"
    "每项必须严格使用以下格式（key 从 values/communication/emotion/lifestyle/"
    "appearance 中选取，label 为对应中文维度名）：\n"
    ...
    "不要使用 dimension、weight 等其它字段名。个人画像时返回空数组 []。\n"
    ...
    "写成「这段时间你……」的回顾口吻，不要复述该版本的字段清单。"
    "如果没有历史数据，返回空数组 []。\n"
    "- conclusion：写在最后的概括性总结，50-120字，第二人称。它是整份画像读完后"
    "的收束：把标题、洞察、维度收拢成一个温柔的整体印象，像朋友合上笔记本前"
    "说的最后一句话。写法要求：把「你最看重的两三件事」与「你期待的生活状态」"
    "轻轻收在一起，结尾留一点余味；不新增前面没提过的信息，不承诺关系结果，"
    "不以建议、口号或行动号召结尾。每次必须围绕本次画像的具体内容重新组织，"
    "不要套用任何固定句式。"
)
...
def build_profile_narrative_prompt(
    ...
    return (
        ...
        f"如果是写「你期望的另一半」，请填写 ideal_weights，"
        f"且所有文案都从「你」的视角出发（你在意什么、你期待怎样的人），"
        f"而不是描述一个第三方。"
        f"如果是写你自己，ideal_weights 返回空数组。"
    )
