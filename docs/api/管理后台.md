...
变更记录：2026-08-17，举报和举报申诉列表的 `status` 按整数范围校验，支持标准 URL 写法如 `?status=1`。
...
审核完成后服务端重新计算所属用户资料完整度。媒体初始审核状态由上传接口写入 `0`（待审核）；公开资料只返回 `review_status=1` 的媒体，审核中、拒绝或隐藏的媒体不会对外展示，但不会让资料公开的用户整体不可见。重复审核是覆盖操作，以最后一次有效请求为准，不提供幂等键和审核版本号。
...
红娘申请审核通知不属于本举报接口，见 `docs/api/注册身份.md`。
...
- 新增红娘牵线服务申请查询、分配和处理接口，详见 `docs/api/红娘服务.md`。
- 红娘申请审核结果新增申请人站内通知，详见 `docs/api/注册身份.md`。
