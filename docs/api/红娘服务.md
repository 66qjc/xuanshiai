...
- 超级管理员查询、分配和处理牵线服务申请。
- 红娘申请提交、我的申请和管理员审核继续使用 `docs/api/注册身份.md` 中的接口。

二期范围：人脸第三方接入、会员权益扩展、积分兑换红娘服务、AI 红娘和实时媒体能力。

> **业务边界（2026-07-24）**：红娘商业化服务只有 `1=付费牵线` 和 `3=私人定制`，新用户没有免费红娘次数，红娘服务次数只能现金购买。用户之间的“申请认识”属于 `docs/api/社交消息.md` / `docs/api/发现.md` 的独立流程，不得复用本模块的服务订单。
