...
- 购买接口只创建待支付订单；会员必须在服务端完成支付回调验签后开通。
- 通过 `WECHAT_PAYMENT_MODE=mock` 可在 development/testing 中使用模拟回调开通会员；生产环境必须设置为 `real`。
...
会员购买接口会创建待支付订单；`WECHAT_PAYMENT_MODE=mock` 时可使用模拟回调完成测试开通，`real` 时必须接入真实微信支付验签后才会开通。
