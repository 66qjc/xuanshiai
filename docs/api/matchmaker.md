...
- 一期不提供免费红娘服务次数；只能通过现金订单购买 `service_type=1` 付费牵线或 `service_type=3` 私人定制。旧 `service_type=2` 不出现在公开商品接口，也不能创建新订单。
- 会员不限次数牵线一期不生效，后续如产品变更必须新增会员权益和额度契约，并同步更新前端迁移说明。
...
3. 确认已支付订单开通后返回 `201`、`status=0`，且 `service_type` 只能为 `1` 或 `3`；不得使用免费服务类型。
